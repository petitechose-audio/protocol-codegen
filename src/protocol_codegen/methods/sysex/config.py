...
class SysExFraming(BaseModel):
    ...
    start: int = Field(
        default=0xF0, ge=0x00, le=0xFF, description="SysEx start byte (MIDI standard: 0xF0)"
    )

    end: int = Field(
        default=0xF7, ge=0x00, le=0xFF, description="SysEx end byte (MIDI standard: 0xF7)"
    )
    ...
    device_id: int = Field(
        default=0x01, ge=0x00, le=0x7F, description="Device identifier (MIDI 7-bit: 0x00-0x7F)"
    )
...
class SysExConfig(BaseModel):
    ...
    framing: SysExFraming = Field(
        default=SysExFraming(), description="SysEx framing bytes and identifiers"
    )

    structure: SysExStructure = Field(
        default=SysExStructure(), description="Message structure offsets"
    )
